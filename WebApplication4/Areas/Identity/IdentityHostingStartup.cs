            ...
            builder.ConfigureServices((context, services) => {
                services.AddDbContext<WebApplication4Context>(options =>
                    options.UseSqlServer(
                        context.Configuration.GetConnectionString("WebApplication4ContextConnection")));

                services.AddDefaultIdentity<WebApplication4User>(options => options.SignIn.RequireConfirmedAccount = true)
